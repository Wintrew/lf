...
Basic support - printf formatted output with variable access / 基础支持 - 带变量访问的printf格式化输出

Full support (requires g++ compiler) - Complete C++ syntax including variables, functions, control structures / 完整支持（需要g++编译器） - 完整的C++语法，包括变量、函数、控制结构
...
cpp.cout << "Full C++ support: " << message << endl; / 完整C++支持
JavaScript Code Blocks / JavaScript 代码块
...
lf
java.System.out.println("Hello from Java"); / 来自Java的问候
PHP Code Blocks / PHP 代码块
Syntax / 语法： php.PHP code
...
lf
php.echo "Hello from PHP"; / 来自PHP的问候
Rust Code Blocks / Rust 代码块
Syntax / 语法： rust.Rust code
...
lf
rust.println!("Hello from Rust"); / 来自Rust的问候
...
这将生成 program.lsf 和 program.lfp (压缩包文件)

Execution / 执行
bash
python lf-run.py program.lsf        # Execute LSF file / 执行LSF文件
python lf-run.py program.lfp        # Execute package file / 执行压缩包文件
python lf-run.py --shell            # Start interactive shell / 启动交互式shell

🔧 Installation Requirements / 安装要求
Required for basic functionality (Python, C++ printf) / 基础功能（Python，C++ printf）所需：
- Python 3.6+

Optional for full language support / 完整语言支持（可选）：
- C++: g++ compiler / g++编译器
- JavaScript: Node.js / Node.js
- Java: JDK / JDK
- PHP: PHP interpreter / PHP解释器
- Rust: Rust toolchain / Rust工具链

To install all required environments, run: / 要安装所有必需环境，请运行：
...
Optimized Runtime - No external compiler dependencies for basic functions / 优化的运行时 - 基础功能无外部编译器依赖

Enhanced C++ Support - Full C++ syntax execution with cross-language variable access (requires g++ compiler) / 增强的C++支持 - 完整C++语法执行和跨语言变量访问（需要g++编译器）

Package Support - Create compressed packages with separate source files / 包支持 - 创建包含独立源文件的压缩包

Multi-language Support - Support for C++, Python, JavaScript, Java, PHP, and Rust / 多语言支持 - 支持C++、Python、JavaScript、Java、PHP和Rust

Real-time Execution - Execute code in their native environments (requires language interpreters/compilers) / 实时执行 - 在原生环境中执行代码（需要语言解释器/编译器）

Cross-language Variable Sharing - Variables defined in one language can be accessed by others / 跨语言变量共享 - 一种语言中定义的变量可被其他语言访问

📦 Package Format / 压缩包格式
The .lfp (LF Package) format contains: .lfp (LF压缩包) 格式包含：

- Source files for each language in their native format 每种语言的原生格式源文件
- A manifest.json file with execution instructions 包含执行指令的 manifest.json 文件
- The original .lsf file 原始 .lsf 文件
...
No external compiler dependencies / 无外部编译器依赖
